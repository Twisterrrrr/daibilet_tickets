...
## 15.04.2026 — Admin V3: Логи (MVP на AuditLog)

### Наблюдения

- Нужна точка расследования «кто что менял» без доступа к инфраструктурным логам и без выгрузки секретов/PII.
- В системе уже есть `AuditLog` + `AuditInterceptor`, но не было удобной операционной страницы.
...
- На MVP показываем только аудит; логи платежей/вебхуков — отдельные вкладки/эндпоинты (в бэклоге).
...
- В карточке события нужно видеть **будущие слоты** в админке без редактирования; дефолтный диапазон `GET .../sessions` (≈30 дней вперёд) скрывал более поздние даты.

### Решения

- **Frontend (`EventScheduleTab`):** запрос с `from=сейчас`, `to=+365` дней (лимит бэкенда); колонки «Осталось», «В продаже» (активен / пауза), счётчик доступных вперёд; чекбокс показа отменённых; явный баннер **только просмотр**; опционально сверка с `scheduleSummary` с детали.
- **Backend:** в DTO строки сеанса (`AdminEventSessionRowDto`) и в `buildAdminSessionRow` добавлено **`isActive`**, чтобы отличать паузу от отмены; `getSessionsRange` передаёт `isActive` из `EventSession`.
...
## 14.04.2026 — Taxonomy (Subcategories) + SEO Audit: единый слой структуры и качества витрины (master plan)

### Наблюдения

- По мере роста доменов (events/venues/articles/collections/landings/promo) качество витрины начинает “расползаться”: часть сущностей публикуется без достаточного контента, часть страниц становится thin/неиндексабельной, а структура каталога теряет предсказуемость.
- Отдельная реализация “подкатегорий” без контроля качества превращается в справочник ради справочника, а отдельный “SEO Audit” без таксономии видит симптомы, но не объясняет структурную причину.

### Решения

- Зафиксирован master plan, где taxonomy и SEO audit проектируются как единая связка:
  - taxonomy отвечает на «что это за сущность»
  - audit отвечает на «достаточно ли хорошо она оформлена и используется»
- SEO Audit выбран как read‑heavy operational layer с понятной read‑моделью (summary + issues list + entity issues) и возможностью перейти к snapshot/caching по мере роста объёма.
- Определены стартовые группы доменных проверок (events/venues/articles/collections/landings/promo/subcategories) и набор issue codes/severity, включая taxonomy‑completeness и taxonomy‑misuse.
- Документ: `docs/SeoAudit-Taxonomy-MasterPlan.md`.

### Проблемы

- Реализация должна быть **эволюционной** и не ломать текущие контракты: первыми шагами считаются read‑поверхности и UI‑триаж, а не авто‑исправления.
- Нужна явная политика, где проходит граница между publish‑gate (жёсткий блок) и audit (операционный контроль), особенно для thin/indexability‑правил.

## 06.04.2026 — Импорт: авто-категоризация + links-first подкатегории для снижения рутины модерации

### Наблюдения

- Экран `Source categories` в админке полезен как “аварийный” инструмент, но в ежедневной модерации создаёт лишнюю рутину: после импорта редактору приходится вручную “сажать” событие в таксономию.
- В системе уже есть links-first модель (`event_subcategory_links`), и publish-gate/подборки/related опираются на links; значит, логично проставлять links уже на импорте.

### Решения

- Импортёры **TC/TEP** при синхронизации автоматически проставляют `event_subcategory_links`:
  - **PRIMARY**: выбирается из результата классификатора; если сигналов нет — safe default по `EventCategory`.
  - **UNIVERSAL secondary** (best-effort, до 3): на текущем этапе ограничено только кодами, которые реально присутствуют в БД (`FAMILY`, `INDOOR`, `OUTDOOR`, `WATER`), чтобы не ломать импорт.
- `Source categories` остаётся в “Позже/Техническое” и используется редко (когда появляются новые/грязные строки внешней категории и хочется их закрепить явно).

### Проблемы

- В текущей БД отсутствует слой `SubcategoryLayer.PRIMARY` (все коды сидятся как `SECONDARY`). Добавлен безопасный fallback PRIMARY→SECONDARY при назначении, но для “чистой” модели нужно досидить PRIMARY-строки отдельной задачей, не смешивая с MVP.
...
## 06.04.2026 — Supplier: мягкое отключение продаж + заморозка обмена

### Наблюдения

- Нужен безопасный способ “поставщик в пассивном режиме”: не удалять данные и не ломать ссылки (без 404), но выключать покупки и при необходимости останавливать B2B обмен.

### Решения

- **Sales OFF:** `Operator.isActive = false` — публичные карточки событий остаются доступны, но offers не выдаются и checkout режет покупку.
- **Exchange frozen:** `Operator.status = SUSPENDED` — Partner API (API keys) не обслуживается.
- Админка: быстрый переключатель `isActive` в legacy списке поставщиков; в admin-v2 — controls для `isActive` и freeze (для фиксации UX/контракта).

### Проблемы

- Глобальная синхронизация источников (TC/TEP) не персонифицирована по поставщику; “заморозка” сейчас про B2B Partner API. При необходимости per-supplier sync gate — отдельная задача.
...
- После слоя назначения подкатегорий (PRIMARY/SECONDARY + links) нужен продуктовый слой: автоподборки и SEO без дублирования фильтрации с каталогом.

### Решения

- Введены `SubcategoryCollectionsService` (события через `CatalogService.getEvents`, площадки отдельным запросом по `subcategoryLinks`) и `SubcategoryLandingService` (шаблонный SEO, публикация только при `isLandingEnabled` + порогах env). Кэш Redis для подборок и для опубликованных лендингов; неопубликованные лендинги не кэшируются.
- Публичные маршруты: `GET /collections/subcategories/events|venues`, `GET /landings/subcategories/:city/:slug`; админ-превью: `GET /admin/subcategory-collections/preview/*`.
- Витрина: при отсутствии материализованного `catalog/landings` для пары город+slug пробуется публичный subcategory-landing (`SubcategoryLandingView`).
- Related: `fetchRelatedEvents` учитывает пересечение кодов по links и расширяет пул кандидатов; `getRelatedVenues` приоритизирует общие подкатегории.

### Проблемы

- `GET /landings/subcategories` без хвоста по-прежнему может пересечься с `GET /landings/:slug` (slug=`subcategories`) — крайний случай; при необходимости зарезервировать slug в CMS.

## 03.04.2026 — Admin/Supplier V2: аудит документации и вход в эпик Event + Sessions

### Наблюдения

- Стратегия и долг по Admin V2 / Supplier V2 перенесены в `Project.md` и `Tasktracker.md`, пофазовый план — в Cursor (`admin_v2_roadmap_*.plan.md`), но в `Diary.md` не было записи — для аудита решений не хватало трассируемости (на продуктовое поведение это не влияет).
- Эпик **событие + расписание сеансов** — ядро продукта; риск зафиксировать неправильную архитектуру при старте «с экранов» без общей модели.

### Решения

- **Event + Sessions = отдельный продукт**, не итерация «оболочки» V2 и не наращивание вкладки ради паритета.
- **Общий слой admin + supplier:** один UX-замысел и **один контракт** сущности события/сеанса; различие — **роли (RBAC)** и объём доступных полей/действий, а не два независимых продукта.
- **Без «быстрых форм»:** не целиться в упрощённый CRUD событий и «простой список сеансов» как финальную форму — иначе неизбежен полный перепис.
- **Старт эпика — вариант A (продуктовый):** сначала описать, **как** создаётся событие, **как** задаётся расписание и **как об этом думает пользователь**; вариант B (сразу UI от текущих API) сознательно отвергнут как опасный.
- Зафиксированы перекрёстные ссылки: `docs/Project.md` § Admin V2 и ЛК поставщика V2; `docs/Tasktracker.md` § Admin / Supplier V2 — архитектурный долг; детализация фаз и границ — план Cursor.

### Проблемы

- ~~Отдельного документа-спека по сценариям Event + Sessions в `docs/` пока нет~~ — закрыто записью ниже и `Event-Sessions-Product-Contract.md`.

---

## 03.04.2026 — Event + Sessions: продуктовый контракт (вкладки, квоты, режимы расписания)

### Наблюдения

- Обсуждение эпика вышло за рамки «списка полей»: нужны режимы расписания, многоуровневые квоты, групповые категории, open date без слотов, типизированный и блочный контент, общий UX для admin/supplier.

### Решения

- Зафиксирован документ **`docs/Event-Sessions-Product-Contract.md`**: вкладки (Информация, Категории и цены, Расписание, Продажи, Медиа, SEO, Качество); продажа **по категориям без рассадки**; квоты трёх уровней (событие → переопределение сеанса → опционально категория); групповые билеты (`groupSize`, списание N с квоты сеанса); дни недели для категории как **фильтр видимости**; **oldPrice** только для витрины; разовое событие — упрощённый UI при одном слоте; повторяющиеся сеансы — **таблица + календарь** как два view одних данных; **open date** без сеансов, валидация по покупке; **запрет смешивать** open date и обычные сеансы в одном событии; опциональные `ContentBlock`; усечённый мастер для **площадки**; список рисков (квоты, oversell, UX «пропала категория»).
- В **`Project.md`** добавлена ссылка в перечень центральных документов.

### Проблемы

- Зависимые категории, отмены/переносы и уведомления клиентов — в контракте отмечены как следующие слои; детализация до сценариев checkout и уведомлений.

## 13.04.2026 — Admin V3 + финализация типизации каталога (sections + PRIMARY) и контур review

### Наблюдения

- Админский UX сильно зависит от “чистой” классификации: без единой матрицы PRIMARY и стабильного derived section фильтры/SEO/модерация начинают расходиться между источниками (TC/TEP) и legacy данными.
- В прод-данных неизбежно есть **смешанное состояние**: часть событий уже на links‑first подкатегориях, часть — на legacy enum (и часть связей может быть inactive/legacy).

### Решения

- Зафиксировали **канонический список PRIMARY (EVENT_ONLY)** и покрыли его маппингом в **5 derived sections** через единый `section-map` (включая compat для legacy/inactive, чтобы старые события не выпадали из section).
- В Admin V3 довели UI до корректной работы со смешанными данными:
  - active canonical PRIMARY доступны для выбора;
  - legacy/inactive не предлагаются в options, но отображаются если уже выбраны (read-only + warning/legacy).
- Протянули в admin summary derived‑поля для модерации классификации:
  - `classificationSource = LINKS | LEGACY_ENUM | NONE`
  - `classificationNeedsReview` (по качественным issue-кодам)
  и показали их в Event Detail (SummaryStrip), чтобы “почему нужно ревью” было прозрачно.

### Проблемы

- Остался хвост “data cleanup”: миграция/скрипт backfill legacy enum → links и зачистка неактивных связей требуют отдельного плана и запуска на данных (не смешивать с UI/импортом).

---

## 03.04.2026 — Event vs Venue: не сливать сущности

### Наблюдения

- Вариант «единый продукт EVENT/VENUE с общими вкладками и скрытым расписанием» удобен на уровне UI-оболочки, но **ломает домен**: страница площадки может быть активной **без** собственных билетов, при этом **события** (выставки и т.д.) живут как отдельные продукты с привязкой к локации.

### Решения

- **Venue** — контейнер: контент, медиа, SEO, связанные события, опционально собственные продажи; **публикация venue ≠ публикация event**.
- **Event** — отдельный продаваемый/витринный продукт: расписание или open date, категории, квоты, билеты, **ссылка на venue**.
- Отвергнуто: **venue как режим события** в одном мастере; эпик **Event + Sessions** **не** расширяется на полный редактор площадки — редактор **Venue** выделен отдельно, с переиспользованием только **оболочки** (вкладки, SEO, медиа, качество, блоки), без механического переноса расписания и квот слота.
- Обновлён **`docs/Event-Sessions-Product-Contract.md`**: §6–8 (разделение сущностей, черновик вкладок venue, границы эпика), §10 п.6 (риск путаницы), §11 (следующий шаг по venue-доку).

### Проблемы

- Отдельного файла `Venue-Editor-Product-Contract.md` (или раздела в архитектурном документе) пока нет — указано как следующий шаг в контракте Event.

---

## 03.04.2026 — Домен Venue/Event, витринные типы площадки и стратегия сидов

### Наблюдения

- Prisma и публичные поверхности уже разводят **Venue** (контейнер/хаб) и **Event** (продукт с `dateMode`, сеансами/offers); **EventOffer.venueId** — мост к продаже «на площадку». Продуктовый контракт не менял схему, а защитил эпик от слияния сущностей в одном мастере.
- Текущий **`seed-venue-links.ts`** даёт склейку по keywords и демо OPEN_DATE, но **не** наполняет осмысленной **программой выставок** для проверки UX и Event + Sessions.

### Решения

- В **`Event-Sessions-Product-Contract.md`**: в шапке зафиксирована роль документа (продуктовый язык поверх домена); **§6.5** — два **витринных типа** площадки (A: место как главная сущность; B: хаб программы; пограничный случай — концертная площадка); **§12** — разделение **базового сида** и **сценарного** с перечнем целевых кейсов.
- В **`Tasktracker.md`** добавлен якорь **`seed-scenario-venue-events`**.
- Правило эпика: **Event Master + Session Editor только для Event**; редактор Venue отдельно, общие UI-паттерны без слияния flow.

### Проблемы

- Реализация сценарного сида не начата; до неё программа площадки в dev остаётся слабо предсказуемой по данным.

---

## 03.04.2026 — Приоритет эпика Event + Sessions: сначала создание + сеансы

### Наблюдения

- Контракт события (вкладки, квоты, open date, категории) описан целиком, но внедрение должно опираться на **устойчивое ядро**; иначе сценарии и сиды не проверяют продукт.

### Решения

- Зафиксирован приоритет в **`Event-Sessions-Product-Contract.md` §11:** **P0** — механика **создания/редактирования события и расписания (sessions)** для **SCHEDULED**; затем категории/checkout и **OPEN_DATE** в рамках того же эпика Event; редактор Venue и сценарный сид не блокируют P0.

### Проблемы

- Нет отдельного покомпонентного плана задач (только приоритет в контракте) — вынести в Tasktracker при старте разработки.

---

## 03.04.2026 — Roadmap execution: URL-каталог, сеансы, Tasktracker

### Наблюдения

- Для обзора сеансов в админке нужен режим «только проблемные» без полного SQL по агрегатам продаж: жёсткий OR по всем типам `issues` в БД был бы тяжёлым и дублировал бы логику флагов.

### Решения

- Канон ссылок на `/events` вынесен в `catalog-events-url.ts` и подключён по сайту/sitemap/хабам (см. Tasktracker `roadmap-s1a-url-module`).
- `GET /admin/events/sessions/overview`: параметр `issuesOnly` — увеличенный `take` по времени (до 1500 строк, `max(take+1, take*6)`), затем фильтр строк с непустым `issues` и обрезка до `take`; флаг `truncated` учитывает и упор в лимит выборки, и наличие дополнительных строк с замечаниями. В админке — переключатель «Только с замечаниями».
- В `Tasktracker.md` добавлены якоря roadmap-s1…s3 для закрытых и открытых пунктов.

### Проблемы

- При `issuesOnly` возможны «пустые» ответы при редких замечаниях в широком окне — оператору нужно сужать период или город; полный обход периода без лимита не делали намеренно.

---

## 03.04.2026 — Дорожная карта: 3 спринта (каталог → admin-v2/ЛК → поставщик + YooKassa)

### Наблюдения

- Нужен явный порядок работ: сначала редакторский контур и консистентность каталога, затем мост v2↔legacy, затем платежный сквозняк и масштаб SEO — иначе растёт риск блокировок команды и шумных платёжных тестов.
...
- Календарные даты старта спринтов не привязаны к релизному циклу — уточнить при планировании.
...
- Витрина салюта 9 мая была реализована как отдельный Next-мультилендинг `/salute-9-may/[citySlug]`, без отражения в `LandingPage` и admin/landings.
- Для сезонных событий (9 мая, Новый год, городские фестивали) нужен единый механизм: **лендинг как сущность в БД + окно видимости по датам**, а не ручное отключение ссылок.
- Материализатор лендингов доработан только по порогу событий (`minEvents`), но не учитывал календарный сезон.

### Решения

- «Салют 9 мая» переведён в полноценные `LandingPage` c `templateType = SEASONAL_EVENT` и slug `salute-9-may` для 4 городов (Москва, СПб, Казань, Нижний Новгород); данные живут в сиде и админке, а публичный канон — `/cities/:citySlug/salute-9-may`.
- Добавлено поле `seasonalPayload` в `LandingPage` и JSON-схема `SeasonalPayloadSchema` (точки обзора, советы, `seasonWindow { startMonthDay, endMonthDay }`).
- `LandingMaterializerService` теперь учитывает сезон: лендинг активен только если `eventsCount ≥ minEvents` **и** текущая дата лежит в `seasonWindow` (поддержаны окна через границу года вроде `12-01`–`01-10`).
- Хаб `/salute-9-may` стал индексом по городским лендингам (читает `/catalog/landings/*/salute-9-may`), редиректы старых URL `/salute-9-may/:citySlug` ведут на канон `/cities/:citySlug/salute-9-may`.
- В admin/landings добавлено поле «Сезонный контент (JSON)» для `seasonalPayload`, так что будущие событийные лендинги (Новый год и др.) можно настраивать без миграций.

### Проблемы

- Пока `seasonWindow` определяется только в seasonalPayload и не визуализируется в UI (нет явного превью периода на странице списка лендингов) — для первых релизов принято как допустимый долг.
- Для других событийных лендингов потребуется дисциплина: ставить `templateType = SEASONAL_EVENT` и заполнять `seasonWindow`, иначе материализатор будет считать лендинг круглогодичным.

---

## 30.03.2026 — Лендинги: хабы, салют, MSK-даты, UX фильтров и коммит

### Наблюдения

- Канонические лендинги остаются на `/cities/:slug/:landingSlug`; параллельно появились **хабы** (`/river-cruises`, `/bus-tours`) и **сезонная витрина** `/salute-9-may` с городскими подстраницами.
- Поведение «сегодня/завтра» и сопоставление сеансов с календарным днём должны совпадать с **Europe/Moscow** независимо от TZ сервера/CI.
- Сортировка на лендинге с таблицей сравнения удобнее в виде **табов с подчёркиванием**, а не `<select>`.
...
- В одном коммите оказались смежные изменения рабочей копии (backend/seed/supplier и др.); при необходимости историю можно расщепить на тематические коммиты постфактум через interactive rebase.
...
- В `Tasktracker.md` блоки `UA-3` и `UA-4` оставались незакрытыми, хотя основные поставки по dashboard/list-паттерну уже интегрированы.
- Пользователь подтвердил переход к следующему этапу (`UA-5`, далее `UA-6`, `UA-7`) с фокусом на 3 вкладки аналитического дашборда.

### Решения

- В трекере формально закрыты `UA-3` и `UA-4` (`[x]`), статус общего блока админ-перехода на единый паттерн обновлен на промежуточный (`[~]`).
- Зафиксирован следующий рабочий поток: `UA-5 Dashboard Analytics Tabs` -> `UA-6 Unified Lists & Forms` -> `UA-7 Final UX/Contrast/Localization`.
...
## 24.03.2026 — Риски Dashboard Analytics MVP зафиксированы

### Наблюдения

- После внедрения `GET /admin/dashboard/analytics-tabs` появился отдельный аналитический слой внутри админского дашборда.
- Обнаружены риски: смешение UX и аналитики, runtime-агрегаты без кэша, потенциальный drift KPI между экранами.

### Решения

- В `Tasktracker.md` добавлен отдельный трек `AN-1 — Dashboard Analytics Hardening`.
- Зафиксирована Analytics policy:
  - `analytics-tabs` = временный MVP endpoint,
  - не source of truth для биллинга/выплат/официальной отчетности,
  - новые метрики только по явному согласованию,
  - ограничение по scope: 8–10 агрегатов, без тяжелых JOIN-цепочек.
- В `Tasktracker.md` добавлен список источников метрик (source/type/usage) для предотвращения KPI drift.

### Проблемы

- Риск деградации времени ответа при росте данных сохраняется до выполнения `AN-1` (кэш, оптимизация, SLA).
...
- Для B2B-поставщиков (Partner API и аналоги) решено не использовать внешний виджет/редирект, а дать тот же UX, что для собственных (MANUAL) событий.

### Решения

- **Форма:** наша — выбор даты/сеанса, типы билетов, количество (как на странице события для MANUAL с BuyModal/date picker).
- **Остатки мест:** подтягиваются по API поставщика при отображении сеансов и при необходимости перед созданием заказа; не храним как единственный источник истины в нашей БД.
- **Платежный контур:** полностью наш — createPackage → YooKassa → FulfillmentItem; после оплаты передача заказа поставщику по контракту (webhook/API).

Зафиксировано в `Project.md`, раздел «Partner B2B API» → «B2B Checkout — наша форма и наш платёжный контур».

### Проблемы

- Нет. Реализация «остатки по API» и контракт передачи заказа — в бэклог.
...
- Backend: tsc OK, 331 тестов passed.
- Frontend (frontend, frontend-admin, frontend-supplier): tsc OK.
- ESLint: 11 ошибок no-unused-vars в backend, frontend-admin, shared-ui.

### Решения

- catalog.controller.ts: `err` → `_err` (caught unused).
- schedule-sync.adapter.ts: `desiredSet` → `_desiredSet`.
- CityEdit.tsx: удалены неиспользуемые импорты Card*.
- SeoAuditPage.tsx, SettingsPage.tsx: удалён неиспользуемый Skeleton.
- EventBasicsStep.tsx: `cities` → `_cities`.
- EventWizard.mappers.ts: `schedule` → `_schedule`.
- Осталось 2 предупреждения any в EventWizard.mappers — допустимо.
...
- Этапы 3–11 завершены: Backend Public/Supplier/Admin, Frontend Public/Supplier/Admin, тесты, документация.
- frontend-supplier не имел зависимости @daibilet/shared-ui; implicit any в supplier-event-wizard.adapter.ts и EventEdit.tsx.

### Решения

- Добавлена зависимость `@daibilet/shared-ui` в packages/frontend-supplier/package.json.
- Явные типы EventWizardTicketTierDraft, EventWizardDraft в адаптере и EventEdit.
- ReviewModuleAudit: этапы 1–11 формально закрыты; e2e-тесты отложены.
...
- TC/TEPLOHOD события — отзывы должны быть полностью отключены; только MANUAL с owner.
- Поставщик должен иметь возможность отвечать на отзывы и оспаривать их с доказательствами.
- Ответы поставщика требуют модерации до публикации.

### Решения

- **ReviewCapabilityService:** canAcceptReviews(event), isImportedAggregatorEvent(event), requiresSupplierResponse(rating). TC/TEPLOHOD → false; MANUAL + (supplierId|operatorId) → true.
- **Модели:** ReviewSupplierResponse, ReviewDispute, ReviewDisputeEvidence, ReviewActionLog. Review: supplierId snapshot, publishedAt.
- **Supplier API:** list reviews (tab: all/needs_response/disputed/responded), getOne, upsertResponse, submitResponse, accept, createDispute, addEvidence.
- **Admin API:** supplier-responses (approve/reject), disputes (resolve: RESOLVED_KEEP/HIDE/DELETE).
- **Frontend:** supplier Reviews page; admin вкладки «Ответы поставщика», «Оспаривания».
- **Тесты:** review-capability.service.spec.ts — unit-тесты для capability.
...
## 11.03.2026 — Content Model / PageTemplateSpecs MVP+ (гибридная архитектура)

### Наблюдения

- EventOverride.templateData — смесь route, menu, program, rules; нужна чёткая граница: core-колонки vs контентный JSON vs наследуемая refund policy.
- Правила возврата/обмена обязательны для Event и Venue; должны наследоваться от Supplier (Operator).

### Решения

- **Prisma:** VenueRefundPolicyMode, EventRefundPolicyMode; Event: meetingPoint, routeSummary, boardingPierId, disembarkPierId, refundPolicyMode, refundPolicyText; EventOverride: contentTemplateData, meetingPoint, routeSummary, refundPolicyMode, refundPolicyText; Venue: refundPolicyMode, refundPolicyText, venueTemplateData; Operator: defaultRefundPolicyText, defaultRefundPolicyUpdatedAt.
- **Shared:** content-template.types.ts (EventContentTemplateData, VenueTemplateData, Zod-схемы), refund-policy.ts (enum, DEFAULT_REFUND_POLICY_TEXT), page-template-specs.ts (getEventTemplateSpecs, getVenueTemplateSpecs).
- **Backend:** RefundPolicyResolutionService (resolveVenueRefundPolicy, resolveEventRefundPolicy); CatalogService — refundPolicyResolved в event detail; EventOverrideService — merge contentTemplateData, meetingPoint, routeSummary, refundPolicy.
- **DTO/Admin:** OverrideEventDto — contentTemplateData, meetingPoint, routeSummary, refundPolicy*; UpdateVenueDto — refundPolicy*, venueTemplateData; UpdateSupplierDto — defaultRefundPolicyText.
- **Docs:** PageTemplateSpecs.md обновлён; ContentModel.md — core vs content JSON vs refund policy.

### Проблемы

- ageLimit не добавлен (Event.minAge уже есть). Admin UI для contentTemplateData/venueTemplateData — отложено (TODO).
...
## 17.03.2026 — Тестовое событие Buyer Account и страница события (PDP)

### Наблюдения

- Для сценариев Buyer Account и посадочной события нужен один стабильный тестовый кейс, который покрывает:
  - связку Event ↔ Venue с реальным адресом/метро,
  - возраст 0+ (не должен скрываться из‑за falsy‑проверок),
  - теги, оффер с бейджем, 10+ сеансов для UX выбора даты‑времени,
  - реальные/псевдо‑рейтинги, тестовые отзывы, наследуемую политику возврата.
- Гибридная контент‑модель PageTemplateSpecs (`contentTemplateData` + refund policy) уже реализована в backend, но на фронте блоки PDP были частично завязаны на legacy `templateData` и не имели явной структуры под «Особенности / Программа / Маршрут / Меню / Памятка гостя / FAQ».
- В листингах и на PDP использовались разные эвристики для рейтинга (жёсткий псевдорейтинг для некоторых тегов, вручную забитые значения), что усложняло UX‑аудит и тестирование.

### Решения

- **Тестовое событие `test-event-buyer-account`:**
  - Venue «Сквер Достоевского» создан и привязан к событию (город СПб, корректный адрес и метро), в сид‑скрипте `db:seed:buyer-test`.
  - Событие фиксировано на `minAge = 0` и отображается как `0+` в Hero и быстрых карточках, без скрытия по `>0`.
  - Добавлены 1–2 тестовых тега (`test-family`, `test-evening`) для проверки рендера тегов и фильтрации.
  - Для события заведены галерея (`galleryUrls`) и 2 осмысленных тестовых отзыва (через сид `Review.createMany`), пригодные для ручной проверки блока отзывов и бейджа рейтинга.
- **Сеансы и выбор даты‑времени:**
  - В `seed-event-sessions-test.ts` расширен набор слотов до 10 `EventSession` с разными датами и временами; price‑сет совпадает с ценами в BuyCard/BuyModal.
  - На PDP блок «Ближайшие сеансы» показывает максимум 5 ближайших активных слотов; полный набор 10 используется в модалке покупки как база для проектирования гибридного выбора даты‑времени.
  - Для MANUAL‑событий слоты без доступных билетов отображаются как статичная информация (место/время сбора) без кликабельности.
- **Рейтинг:**
  - В `CatalogService` введён helper `getDisplayedEventRating(eventId, rawRating, reviewCount)`:
    - при `reviewCount >= 10` → используется фактический рейтинг события;
    - при `<10` отзывов → детерминированный псевдорейтинг 4.5–5.0, основанный на хеше `event.id`, стабилен для одного и того же события.
  - Helper применяется:
    - в листингах (`getEvents`/`enrichWithBadges`),
    - в детальной странице (`fetchEvent`),
    - в блоке «Похожие события» (relatedEvents),
    - реальные данные в БД (`Event.rating`) не подменяются.
  - На фронте:
    - `EventCard` всегда показывает бейдж рейтинга, если `rating > 0`, без требования `reviewCount >= 10`,
    - `RatingBadge` на PDP отображает псевдо‑ или реальный рейтинг, а количество отзывов — по `reviewCount`.
- **Контентные блоки PDP (PageTemplateSpecs → EventContentTemplateData):**
  - В `EventPageView` реализован компонент `EventContentBlocks`, который читает `contentTemplateData` и `refundPolicyResolved` и рендерит под описанием события строго в порядке:
    ...
    8. «Правила обмена и возврата» (`refundPolicyResolved` c fallback на `bookingRules` при его отсутствии).
  - Каждый блок рендерится только при наличии данных в соответствующем поле; legacy `templateData` остаётся поддержанным, но контент для новых событий должен идти через `contentTemplateData`.
- **Refund policy inheritance:**
  - Подтверждено использование `RefundPolicyResolutionService.resolveEventRefundPolicy` как единственного источника финальной политики возврата/обмена (Supplier → Venue → Event).
  - DTO события расширен полем `refundPolicyResolved`, frontend PDP опирается именно на это поле и больше не реализует собственный слой наследования.
- **Hero / CTA / возраст:**
  - CTA в Hero приведён к «Afisha‑like» виду: компактный бейдж с «ОТ» и ценой (`formatPrice(priceFrom)` с ₽), по клику скроллит к блоку покупки `id="buy-card"`, саму цену «от … / чел.» сохраняем в BuyCard.
  - Возраст 0+ больше не интерпретируется как falsy и не скрывается; в быстрых карточках явно показано `0+` либо «Для всех» при отсутствии значения.

### Проблемы

- Гибридный календарь+слоты в модалке (`BuyModal`) пока не реализован полностью: модалка может работать с 10+ слотов как с данными, но UI календаря по доступным датам и группировка слотов по выбранной дате отложены на отдельную задачу (чтобы не раздувать scope и не ломать текущий checkout‑флоу).
- Для сиедов предусмотрена защита по `EventOverride.subcategoriesOverride` (NOT NULL) — пришлось явно задавать `subcategoriesOverride: []` и `subcategoriesMode: 'INHERIT'` в сид‑скрипте, иначе Prisma падал с P2011.
...
- Письмо получателю при создании подарочного сертификата отправляется по очереди email-процессором (`gift-certificate`).
