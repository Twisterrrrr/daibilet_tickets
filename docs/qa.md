...
# Вопросы по архитектуре Venue geo + Trip routes (QA)

> Цель: зафиксировать продуктовые решения по нормализации района/метро и точкам маршрута без поломки legacy-строк и SEO-каноники.

## 1) Legacy vs FK

- Нужна ли **синхронизация** legacy `Venue.district`/`Venue.metro` при выставлении FK (или оставляем строки «как есть» навсегда и считаем FK источником истины только для отображения)?
- Как обрабатываем конфликт, когда строка и FK «не совпадают по смыслу» (ручная правка в админке vs автозатира)?

## 2) Словари и качество данных

- Достаточно ли **точного** матча (name/slug) для backfill, или нужен отдельный слой ручных алиасов (synonyms) до включения автозаполнения?
- Нужны ли **мульти-метро** для одной площадки (сейчас модель допускает одну станцию через FK)?
