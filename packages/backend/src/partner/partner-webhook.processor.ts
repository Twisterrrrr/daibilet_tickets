...
export class PartnerWebhookProcessor extends WorkerHost {
  ...
  async process(job: Job<WebhookJobData>): Promise<any> {
    ...
    const signature = webhookSecret
      ? PartnerWebhookService.sign(bodyStr, webhookSecret)
      : '';
    ...
    try {
      ...
      if (!response.ok) {
        const text = await response.text().catch((e) => { this.logger.error('webhook delivery failed: ' + (e as Error).message); return ''; });
      ...
      this.logger.warn(
        `Webhook failed: [${payload.event}] → ${operatorName}: ${msg}`,
      );
