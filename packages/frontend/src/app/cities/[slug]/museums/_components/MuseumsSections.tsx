...
export function MuseumsSections({ citySlug, content }: MuseumsSectionsProps) {
  ...
  const main = content.mainMuseums ?? [
  ...
  const modern = content.modernMuseums ?? [
  ...
  const kids = content.kidsMuseums ?? [
  ...
  return (
    <div className="mt-8 grid gap-6 sm:grid-cols-2">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-2 text-sm text-slate-600">Для первого визита и обязательной программы.</div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div className="mt-2 text-sm text-slate-600">Гибкие форматы и апселлы.</div>
        <div className="mt-4 flex flex-wrap gap-2">
          {modern.map((x) => (
            <Link
              key={x.slug}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:col-span-2">
        ...
        <div className="mt-2 text-sm text-slate-600">Интерактивные и семейные музеи.</div>
        <div className="mt-4 flex flex-wrap gap-2">
          {kids.map((x) => (
            <Link
              key={x.slug}
