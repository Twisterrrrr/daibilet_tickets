...
import { formatPrice, Intensity, INTENSITY_LABELS, type CityListItem, type EventListItem } from '@daibilet/shared';
import {
  ArrowLeft,
  ArrowRight,
  Calendar,
...
type TierKey = 'economy' | 'optimal' | 'premium';
...
export default function PlannerPage() {
    ...
    | {
      ...
      days: Array<{
        ...
        slots: Array<{
          ...
          tickets: {
            adult: { count: number; unitPrice: number };
            child: { count: number };
          };
        }>;
      }>;
  ...
  const [replaceEvents, setReplaceEvents] = useState<EventListItem[]>([]);
  ...
  const handleReplaceEvent = async (newEventId: string) => {
    if (!replaceModal || !result || result.error) return;
    ...
    try {
      ...
      if (res.success && res.variant) {
        setResult((prev) => {
          if (!prev || prev.error || !prev.variants) return prev;
          const nextVariants = prev.variants.map((v, i) =>
            i === selectedVariant && res.variant ? (res.variant as (typeof prev.variants)[0]) : v,
          );
  ...
  const calculate = async () => {
    ...
    try {
      ...
      setResult({
        ...res,
        meta: {
          dayCount: (res.meta as { dayCount?: number })?.dayCount ?? res.variants?.[0]?.days?.length ?? 0,
          availableEventsCount: (res.meta as { availableEventsCount?: number })?.availableEventsCount ?? 0,
          message: (res.meta as { message?: string })?.message,
        },
      });
      // Auto-select optimal variant if it exists
      const optIdx = res.variants?.findIndex((v) => v.tier === 'optimal');
  ...
  // Upsell total
  const upsellTotal =
    (result && !('error' in result) ? result.upsells ?? [] : [])
      .filter((u: { id: string; priceKopecks: number }) => selectedUpsells.has(u.id))
      .reduce((s: number, u: { priceKopecks: number }) => s + u.priceKopecks, 0) || 0;
      ...
      <div className={`mx-auto ${step === 'results' ? 'max-w-4xl' : 'max-w-lg'}`}>
        ...
        {step === 'results' && (
          <div className="space-y-8">
            {loading ? (
            ...
            ) : result && !result.error && (result.variants?.length ?? 0) > 0 ? (
              <>
                {/* Header */}
                <div className="text-center">
                  <h2 className="text-2xl font-bold text-slate-900">Ваша программа в {cityName}</h2>
                  <p className="mt-1 text-sm text-slate-500">
                    {(result.meta?.dayCount ?? 0)}{' '}
                    {(result.meta?.dayCount ?? 0) === 1
                      ? 'день'
                      : (result.meta?.dayCount ?? 0) < 5
                        ? 'дня'
                        : 'дней'}{' '}
                    {totalPersons} {totalPersons === 1 ? 'человек' : totalPersons < 5 ? 'человека' : 'человек'} &middot;{' '}
                    {result.meta?.availableEventsCount ?? 0} событий доступно
                  </p>
                </div>

                {/* ===== Tier Cards ===== */}
                <div className="grid gap-4 md:grid-cols-3">
                  {result.variants.map((v, i: number) => {
                    const style = TIER_STYLES[(v.tier as TierKey) || 'optimal'] || TIER_STYLES.optimal;
                    const TierIcon = style.icon;
                    const isSelected = selectedVariant === i;
                    const eventCount = v.days.reduce((s, d) => s + d.slots.length, 0);
                ...
                {/* Savings hint */}
                {(result.variants?.length ?? 0) >= 2 &&
                  (() => {
                    const variants = result.variants ?? [];
                    const cheapest = variants[0];
                    const priciest = variants[variants.length - 1];
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      {variant.days.map((day) => {
                        ...
                        const dayTotal = day.slots.reduce((s, sl) => s + sl.subtotal, 0);
                                        ...
                                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs text-slate-400">
                                          ...
                                          {(() => {
                                            ...
                                            return available > 0 && available < 20 ? (
                                              <span className="text-red-500 font-medium">Осталось {available} мест</span>
                                            ) : null;
                                          })()}
                                        </div>
                                      </div>
              ...
              <div className="rounded-xl border border-red-200 bg-red-50 p-8 text-center">
                <p className="text-red-700">{result.message ?? 'Не удалось подобрать программу'}</p>
                <button onClick={() => { setStep('city'); setResult(null); }} className="btn-secondary mt-4">
                  Начать сначала
                </button>
              </div>
            ) : result?.meta?.message ? (
