...
export default async function CollectionsListPage() {
  ...
  const cityGroups = Array.from(byCity.values()).sort((a, b) => b.items.length - a.items.length);
...
function CollectionCard({ collection }: { collection: CollectionCardVM }) {
  return (
    ...
    >
      ...
      {!collection.heroImage && <div className="absolute inset-0 bg-gradient-to-br from-primary-700 to-primary-900" />}
      <div className="relative z-10">
        <h3 className="text-xl font-bold text-white">{collection.title}</h3>
        {collection.subtitle && <p className="mt-1 text-sm text-white/70">{collection.subtitle}</p>}
