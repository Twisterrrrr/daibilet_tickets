      ...
      <section className="relative bg-gradient-to-b from-slate-900 to-slate-800 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-14">
          ...
          <nav className="flex items-center text-xs text-white/50 gap-1 mb-4">
            ...
            <Link href="/venues" className="hover:text-white/80">
              Музеи и Арт
            </Link>
            {venue.city && (
              <>
                <ChevronRight size={12} />
                <Link href={`/venues?city=${venue.city.slug}`} className="hover:text-white/80">
                  {venue.city.name}
                </Link>
              </>
            )}
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
            <div className="max-w-2xl">
              ...
              {venue.rating > 0 && (
                <div className="flex items-center gap-2 mt-3">
                  <div className="flex items-center gap-1">
                    {[1, 2, 3, 4, 5].map((i) => (
                      <Star
                        key={i}
                        size={16}
                        className={i <= Math.round(venue.rating) ? 'text-yellow-400 fill-yellow-400' : 'text-white/30'}
                      />
                    ))}
                  </div>
                  ...
                  <a
                    href="#reviews"
                    className="text-white/60 hover:text-white/90 text-sm underline-offset-2 hover:underline"
                  >
            ...
            <div className="hidden md:flex flex-col items-end gap-2 flex-shrink-0">
              ...
              {todayHours && <p className="text-xs text-white/50">Сегодня {todayHours}</p>}
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1.5">
                      {DAY_ORDER.map((day) => {
                        ...
                        return (
                          <div
                            key={day}
                            className={`flex justify-between text-sm ${isToday ? 'font-semibold text-blue-700' : 'text-gray-600'}`}
                          >
                            <span>
                              {DAY_NAMES[day]}
                              {isToday ? ' (сегодня)' : ''}
                ...
                <div className="bg-white rounded-xl border border-gray-200 p-4">
                  ...
                  {venue.address && <p className="text-sm text-gray-700 mb-2">{venue.address}</p>}
                  {venue.metro && (
              ...
              {venue.externalRating && venue.externalSource && (
                <div className="flex items-center gap-2 px-5 py-3 bg-gray-50 rounded-xl text-sm">
                  ...
                  <span className="text-gray-500">
                    {venue.externalSource === 'yandex_maps'
                      ? 'Яндекс.Карты'
                      : venue.externalSource === '2gis'
                        ? '2ГИС'
                        : venue.externalSource}
                  </span>
                </div>
              )}
            </div>
