import { Star, Quote, Shield, ArrowRight } from 'lucide-react';
    ...
    <section className="py-12">
      {items.map((item, idx) => (
        <div key={idx} className="mb-6 last:mb-0">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-relaxed text-slate-600 whitespace-pre-line">
            {item.text}
          </p>
        </div>
      ))}
      ...
      {stats.soldTickets > 0 && (
        <div>
          <div className="text-2xl font-bold text-primary-700">
            {stats.soldTickets.toLocaleString('ru-RU')}+
          </div>
          <div className="text-xs text-slate-500">
            билетов продано за сезон
          </div>
        </div>
      )}
      {stats.avgRating > 0 && (
