...
// --- API Response Types (frontend api.ts) ---
...
export interface CityDetail extends CityListItem {
  ...
  stats?: { excursionCount: number; museumCount: number; eventCount: number; totalCount: number };
  popularTags?: Array<TagItem & { _count?: { events: number } }>;
  landingPages?: Array<{ slug: string; title: string; subtitle?: string }>;
  regionPreview?: unknown;
}
...
export interface TagDetailPage {
  tag: TagItem & { metaTitle?: string | null; metaDescription?: string | null; description?: string | null };
...
/** Поля лендинга для страницы (контентные блоки) */
export interface LandingDetailContent {
...
/** Ответ GET /landings/:slug — лендинг с вариантами и фильтрами */
export interface LandingBySlugResponse {
  landing: LandingListItem & { city: { slug: string; name: string } } & LandingDetailContent;
...
export interface ReviewResponse {
  ...
  externalReviews?: Array<{ id: string; source: string; sourceUrl?: string; rating?: number; publishedAt?: string }>;
  summary?: { avgRating: number; reviewCount: number; verifiedCount: number; distribution?: Record<number, number> };
  total: number;
  page: number;
  totalPages: number;
}
